...
if(!empty($_POST['name']) && !empty($_POST['tel'])){
  ...
  if($rq['note']!='') $desc .= '<strong>Yêu cầu:</strong> '.nl2br($rq['note']).' <br>';
  if($rq['date']!='') $desc .= '<strong>Ngày:</strong> '.$rq['date'].' <br>';
  ...
  if($dx->query($s)) {
    ...
    $message = 'Thông tin nhận từ '.MAIN_DOMAIN.'<br>
      - Họ tên: '.$rq['name'].'<br>
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-162704093-8"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-162704093-8');
</script>
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-6 col-12" >
          <img src="https://dodungkhachsanphuquoc.com/files/banner-trang-chu-1-585R5CUlFa.png" alt="Đức Trí">
        </div>
        <div class="col-md-6 col-12">
          <h3>Trân trọng!</h3>
          <p>Cảm ơn quý khách đã tin tưởng và lựa chọn Omely. Chúng tôi sẽ phản hồi lại trong thời gian gần nhất.</p>
